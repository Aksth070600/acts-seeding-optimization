...
# Pixel-only seeding driver against a GNN4ITk Athena dump.
...
SPACE_POINTS = "pixel_spacepoints"
OUTPUT_SEEDS = "pixel_seeds"
ITK_SP_TYPE = InputSpacePointsType.PixelSpacePoints
HIGH_OCCUPANCY = True


def main():
    parser = argparse.ArgumentParser(
        description="Run Athena-dump pixel seeding with a selectable seeding algorithm",
    )
    ...
    if args.parameters:
        raise ValueError(
            "PixelSeeding does not accept --parameters. "
    ...
    if args.version not in adders:
        raise ValueError(
            f"PixelSeeding only supports {[v.value for v in adders]}; "
    ...
    adders[args.version](
        s,
        SPACE_POINTS,
        *itkSeedingAlgConfig(ITK_SP_TYPE, highOccupancyConfig=HIGH_OCCUPANCY),
        logLevel=ll,
