...
ProcessCode SphericalGridTripletSeedingAlgorithm::execute(
  ...
  const std::size_t nPhi =
      sphericalGrid->numberOfBins() / ((nEta + 2u) * (nR + 2u));
  ...
  for (const auto& [bottom, middle, top] : sphericalGrid->binnedGroup()) {
    const auto sz = sphericalGrid->at(middle).size();
    num_bins(1);
    if (sz > 0) {
      occupied_bins(1);
    }
  ...
  Acts::SpacePointContainer2 coreSpacePoints(
      ...
      Acts::SpacePointColumns::ZR | Acts::SpacePointColumns::EtaR |
  ...
  for (const auto [bottom, middle, top] : sphericalGrid->binnedGroup()) {
    ...
    const auto seedsBefore = seeds.size();
    ...
    seeds_per_group(static_cast<std::int64_t>(seeds.size() - seedsBefore));
  }

  ACTS_DEBUG("Created " << seeds.size() << " track seeds from "
                        << spacePoints.size() << " space points");

  total_seeds(static_cast<std::int64_t>(seeds.size()));
  STATS_DUMP();
