...
void createAndFilterTriplets(TripletSeeder::Cache& cache,
  ...
  for (auto bottomDoublet : bottomDoublets) {
    ...
    triplets_evaluated(
        static_cast<std::int64_t>(cache.tripletTopCandidates.size()));
...
void createSeedsFromGroupsImpl(
  ...
  top_candidates(static_cast<std::int64_t>(cache.topDoublets.size()));
  ...
  bottom_candidates(static_cast<std::int64_t>(cache.bottomDoublets.size()));
