...
/// A spherical space point grid used for seeding.
/// The grid is defined in spherical coordinates (phi, eta, r)
class SphericalSpacePointGrid2 {
 public:
  ...
  /// Type alias for eta axis with variable binning and open boundaries
  ...
  /// Spherical space point grid type, which is a grid over `BinType` with
  /// axes defined by `PhiAxisType`, `EtaAxisType`, and `RAxisType`.
  /// The grid is a 3D grid with the axes representing azimuthal angle (phi),
  /// pseudorapidity, and radial distance (r).
  ...
  struct Config {
    ...
    /// enable non equidistant binning in eta
  ...
  explicit SphericalSpacePointGrid2(
      const Config& config,
      std::unique_ptr<const Logger> logger =
          getDefaultLogger("SphericalSpacePointGrid2", Logging::Level::INFO));
  ...
  /// distance, and eta-coordinate.
  /// @param position The position of the space point in (phi, eta, r)
  ///        coordinates
  ...
  /// @param eta The eta-coordinate of the space point
  /// @param r The radial distance of the space point from the origin
  /// @return The index of the bin in which the space point is located, or
  ///         `std::nullopt` if the space point is outside the grid bounds.
  std::optional<std::size_t> binIndex(float phi, float eta, float r) const {
    return binIndex(Vector3(phi, eta, r));
  }
  ...
  ///         `std::nullopt` if the space point is outside the grid bounds.
  std::optional<std::size_t> insert(SpacePointIndex index, float phi, float eta,
                                    float r);
  /// Insert a space point into the grid.
  ...
  std::optional<std::size_t> insert(const ConstSpacePointProxy2& sp) {
    return insert(sp.index(), sp.phi(), sp.eta(), sp.r());
  }
