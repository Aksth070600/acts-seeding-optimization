...
class DoubletSeedFinder {
 public:
  ...
  struct Config {
    ...
    /// Use spherical coordinates (eta-phi-r) instead of cylindrical (z-phi-r)
    /// When true, the doublet finder will use etaR column for binning
    /// but will still use z directly for all physics calculations
    bool useSphericalCoords = false;
