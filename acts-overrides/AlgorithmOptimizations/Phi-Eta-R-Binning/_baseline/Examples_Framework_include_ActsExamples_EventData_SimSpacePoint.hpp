...
class SimSpacePoint {
 public:
  ...
  /// Calculate and return azimuthal angle phi (in radians)
  double phi() const {
    return std::atan2(m_y, m_x);
  }

  /// Calculate and return pseudorapidity (eta)
  /// Pseudorapidity is defined as: eta = -ln(tan(theta/2))
  /// where theta is the polar angle from the beam axis (z-axis)
  double eta() const {
    // Handle the special case where r = 0
    if (m_rho == 0.0) {
      // Point is on the beam axis
      // Return +infinity for positive z, -infinity for negative z, 0 for z=0
      if (m_z > 0.0) {
    ...
    return std::asinh(m_z / m_rho);
  }

  /// Calculate and return the polar angle theta (in radians)
  double theta() const {
    return std::atan2(m_rho, m_z);
  }
 ...
 private:
  // Global position (Cartesian coordinates - primary storage)
  double m_x;
  double m_y;
  double m_z;
  std::optional<double> m_t;
  
  // Cached transverse radius (r = sqrt(x^2 + y^2))
  double m_rho;
  ...
  // Strip measurement details (for strip detectors)
