...
void TripletSeeder::createSeedsFromGroups(
  ...
  if (spacePointsSortedByRadius) {
    ...
    // Access radius - works for both cylindrical (zr) and spherical (etaR) as both store r at index [1]
    ...
    for (auto& bottomSpGroup : bottomSpGroups) {
      ...
      const auto low = std::ranges::lower_bound(
          ...
          [&](const ConstSpacePointProxy2& sp) { return sp.zr()[1]; });  // Radius access
  ...
  for (ConstSpacePointProxy2 spM : middleSpGroup) {
    // Access radius - same for both coordinate systems
