...
class SpacePointProxy2 {
 public:
  ...
  /// Mutable access to the eta (pseudorapidity) coordinate of the space point.
  /// @return A mutable reference to the eta coordinate of the space point.
  float &eta() const noexcept
    requires(!ReadOnly)
  {
    return m_container->eta(m_index);
  }
  ...
  /// @brief Get mutable reference to EtaR coordinates of the space point
  /// @return Mutable reference to array containing [eta, r] coordinates
  std::array<float, 2> &etaR() const noexcept
    requires(!ReadOnly)
  {
    return m_container->etaR(m_index);
  }
  ...
  /// Const access to the eta (pseudorapidity) coordinate of the space point.
  /// @return The eta coordinate of the space point.
  float eta() const noexcept { return m_container->eta(m_index); }
  ...
  /// @brief Get const reference to EtaR coordinates of the space point
  /// @return Const reference to array containing [eta, r] coordinates
  const std::array<float, 2> &etaR() const noexcept {
    return m_container->etaR(m_index);
  }
