...
SeedingAlgorithm = Enum(
    "SeedingAlgorithm",
    "Default TruthSmeared TruthEstimated Orthogonal HoughTransform AdaptiveHoughTransform Gbts Hashing GridTriplet OrthogonalTriplet SphericalGridTriplet",
)
...
SeedFinderConfigArg = namedtuple(
    ...
    defaults=[None] * 19 + [(None, None)] * 8 + [None] * 3,
)
    ...
    if seedingAlgorithm == SeedingAlgorithm.TruthSmeared:
        ...
        if seedingAlgorithm == SeedingAlgorithm.TruthEstimated:
            ...
            seeds = addSphericalGridTripletSeeding(
                ...
                seedFinderOptionsArg,
                seedFilterConfigArg,
                spacePointGridConfigArg,  # Will be converted internally
                logLevel,
            )
...
def addSphericalGridTripletSeeding(
    ...
    """adds spherical grid triplet seeding
    For parameters description see addSeeding
    """
    logLevel = acts.examples.defaultLogging(sequence, logLevel)()
    ...
    seedingAlg = acts.examples.SphericalGridTripletSeedingAlgorithm(
        ...
        **acts.examples.defaultKWArgs(
            ...
            # Z-coordinate extent (for physics cuts)
            zMin=seedFinderConfigArg.z[0],
            zMax=seedFinderConfigArg.z[1],
            
            # Spherical coordinates (eta instead of z for binning)
            etaMin=seedFinderConfigArg.eta[0],
            etaMax=seedFinderConfigArg.eta[1],
            deltaEtaMin=0.0,  # Usually 0
            ...
            # Eta binning configuration (spherical equivalent of zBinEdges)
            etaBinEdges=seedFinderConfigArg.etaBinEdges,
            etaBinsCustomLooping=seedFinderConfigArg.etaBinsCustomLooping,
            
            # *** CRITICAL: Neighbor bin configuration ***
            ...
            # Z-cuts for doublet formation (in addition to eta cuts)
            deltaZMin=None,
            deltaZMax=None,
