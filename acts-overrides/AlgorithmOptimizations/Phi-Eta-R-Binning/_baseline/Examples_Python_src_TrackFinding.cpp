...
void addTrackFinding(Context& ctx) {
  ...
  {
    // The proxy type is defined by Acts::SpacePointContainer, not by
    // ActsExamples::SpacePointContainer.
    using ExampleSpCollection =
        ActsExamples::SpacePointContainer<std::vector<const SimSpacePoint*>>;
    using ActsSpContainer =
        Acts::SpacePointContainer<ExampleSpCollection, Acts::detail::RefHolder>;
    using SpacePoint = typename ActsSpContainer::SpacePointProxyType;

    using Config = Acts::SeedFinderConfig<SpacePoint>;
...

  {
    ...
    using Config = Acts::SeedFinderOrthogonalConfig<SpacePoint>;
    auto c = py::class_<Config>(mex, "SeedFinderOrthogonalConfig")
                 .def(py::init<>());
  ...
  ACTS_PYTHON_DECLARE_ALGORITHM(
      ...
      "SphericalGridTripletSeedingAlgorithm", 
      inputSpacePoints, outputSeeds,
      bFieldInZ, minPt, cotThetaMax, impactMax,  // ADDED: cotThetaMax
      deltaRMin, deltaRMax, deltaRMinTop, deltaRMaxTop, 
      deltaRMinBottom, deltaRMaxBottom,
      rMin, rMax,
      zMin, zMax,  // ADDED: zMin, zMax (for physics cuts)
      etaMin, etaMax,  // Spherical-specific: eta range for binning
      phiMin, phiMax, phiBinDeflectionCoverage, maxPhiBins,
      etaBinNeighborsTop, etaBinNeighborsBottom, numPhiNeighbors,  // Spherical equivalent of zBinNeighbors
      etaBinEdges, etaBinsCustomLooping,  // Spherical equivalent of zBinEdges
      rMinMiddle, rMaxMiddle, useVariableMiddleSPRange, rRangeMiddleSP,
      deltaRMiddleMinSPRange, deltaRMiddleMaxSPRange,
      deltaEtaMin, deltaEtaMax,  // Spherical-specific: eta cuts
      deltaZMin, deltaZMax,  // ADDED: deltaZMin, deltaZMax (for physics cuts in z-space)
      interactionPointCut, collisionRegionMin, collisionRegionMax,
      helixCutTolerance, sigmaScattering, radLengthPerSeed, toleranceParam,
      deltaInvHelixDiameter, compatSeedWeight, impactWeightFactor,
      zOriginWeightFactor, maxSeedsPerSpM, compatSeedLimit, seedWeightIncrement,
  ...
  ACTS_PYTHON_DECLARE_ALGORITHM(
      OrthogonalTripletSeedingAlgorithm, mex, "OrthogonalTripletSeedingAlgorithm",
      inputSpacePoints, outputSeeds, bFieldInZ, minPt, cotThetaMax, impactMax,
      deltaRMin, deltaRMax, deltaRMinTop, deltaRMaxTop, deltaRMinBottom,
      deltaRMaxBottom, rMin, rMax, zMin, zMax, phiMin, phiMax, rMinMiddle,
      rMaxMiddle, useVariableMiddleSPRange, rRangeMiddleSP,
      deltaRMiddleMinSPRange, deltaRMiddleMaxSPRange, deltaZMin, deltaZMax,
      interactionPointCut, collisionRegionMin, collisionRegionMax,
      helixCutTolerance, sigmaScattering, radLengthPerSeed, toleranceParam,
      deltaInvHelixDiameter, compatSeedWeight, impactWeightFactor,
  ...
  ACTS_PYTHON_DECLARE_ALGORITHM(
      SeedingOrthogonalAlgorithm, mex, "SeedingOrthogonalAlgorithm",
  ...
  ACTS_PYTHON_DECLARE_ALGORITHM(
      TrackParamsLookupEstimation, mex, "TrackParamsLookupEstimation", refLayers,
      bins, inputHits, inputParticles, trackLookupGridWriters);
