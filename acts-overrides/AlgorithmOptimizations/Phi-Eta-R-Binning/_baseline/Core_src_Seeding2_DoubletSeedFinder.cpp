...
template <bool isBottomCandidate, bool interactionPointCut, bool sortedByR,
          bool experimentCuts, bool useSphericalCoords>
class Impl final : public DoubletSeedFinder {
 public:
  ...
  void createDoubletsImpl(const ConstSpacePointProxy2& middleSp,
    ...
    // Select which column to use for iteration based on coordinate system
    // This is compile-time selection with zero runtime overhead
    const SpacePointContainer2& container = candidateSps.container();

    // Select which column to use for iteration based on coordinate system
    // This is compile-time selection with zero runtime overhead
    const auto coordColumn = [&]() {
      if constexpr (useSphericalCoords) {
        return container.etaRColumn(); 
      } else {
        return container.zrColumn();     
      }
     }();

    // Always get z directly for physics calculations
    const auto zrColumn = container.zrColumn();

    for (auto [indexO, xyO, coordRO, zrO, varianceZO, varianceRO] : 
       candidateSps.zip(
           container.xyColumn(),
           coordColumn,              
           zrColumn,             
...
std::unique_ptr<DoubletSeedFinder> DoubletSeedFinder::create(
  ...
  using UseSphericalCoordsOptions = BooleanOptions;
  ...
  boost::mp11::mp_for_each<DoubletOptions>([&](auto option) {
    ...
    using UseSphericalCoords = boost::mp11::mp_at_c<OptionType, 4>;
    ...
    if (configIsBottomCandidate != IsBottomCandidate::value ||
        ...
        config.useSphericalCoords != UseSphericalCoords::value)  {
    ...
    result = std::make_unique<
        Impl<IsBottomCandidate::value, InteractionPointCut::value,
             SortedByR::value, ExperimentCuts::value,
             UseSphericalCoords::value>>(config);
  });
  if (result == nullptr) {
    throw std::runtime_error(
