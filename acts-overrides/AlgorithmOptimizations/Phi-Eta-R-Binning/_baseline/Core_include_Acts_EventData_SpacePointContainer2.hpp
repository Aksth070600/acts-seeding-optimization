...
enum class SpacePointColumns : std::uint32_t {
  ...
  XY = 1 << 15,          ///< X and Y coordinates
  ZR = 1 << 16,          ///< Z and R coordinates
  EtaR = 1 << 17,        ///< Eta and R coordinates
  XYZ = 1 << 18,         ///< X, Y, and Z coordinates
  XYZR = 1 << 19,        ///< X, Y, Z, and R coordinates
  VarianceZR = 1 << 20,  ///< Variance in Z and R directions
...
class SpacePointContainer2 {
 public:
  ...
  /// Returns a mutable proxy to the eta coordinate column.
  /// @return A mutable proxy to the eta coordinate column.
  MutableSpacePointColumnProxy<float> etaColumn() noexcept {
    assert(m_etaColumn.has_value() && "Column 'eta' does not exist");
    return m_etaColumn->proxy(*this);
  }
  ...
  /// Returns a mutable proxy to the etaR coordinates column.
  /// @return A mutable proxy to the etaR coordinates column.
  MutableSpacePointColumnProxy<std::array<float, 2>> etaRColumn() noexcept {
    assert(m_etaRColumn.has_value() && "Column 'etaR' does not exist");
    return m_etaRColumn->proxy(*this);
  }
  ...
  /// Returns a const proxy to the eta coordinate column.
  /// @return A const proxy to the eta coordinate column.
  ConstSpacePointColumnProxy<float> etaColumn() const noexcept {
    assert(m_etaColumn.has_value() && "Column 'eta' does not exist");
    return m_etaColumn->proxy(*this);
  }
  ...
  /// Returns a const proxy to the etaR coordinates column.
  /// @return A const proxy to the etaR coordinates column.
  ConstSpacePointColumnProxy<std::array<float, 2>> etaRColumn() const noexcept {
    assert(m_etaRColumn.has_value() && "Column 'etaR' does not exist");
    return m_etaRColumn->proxy(*this);
  }
  ...
  /// Mutable access to the eta coordinate of the space point at the given index.
  /// @param index The index of the space point.
  /// @return A mutable reference to the eta coordinate of the space point.
  float &eta(Index index) noexcept {
    assert(m_etaColumn.has_value() && "Column 'eta' does not exist");
    assert(index < m_etaColumn->size() && "Index out of bounds");
    return m_etaColumn->proxy(*this)[index];
  }
  ...
  /// Mutable access to the variance etaR of the space point at the given
  /// index.
  /// @param index The index of the space point.
  /// @return A mutable reference to the variance etaR of the space point.
  std::array<float, 2> &etaR(Index index) noexcept {
    assert(m_etaRColumn.has_value() && "Column 'etaR' does not exist");
    assert(index < m_etaRColumn->size() && "Index out of bounds");
    return m_etaRColumn->proxy(*this)[index];
  }
  ...
  /// Const access to the eta coordinate of the space point at the given index.
  /// @param index The index of the space point.
  /// @return A const reference to the eta coordinate of the space point.
  float eta(Index index) const noexcept {
    assert(m_etaColumn.has_value() && "Column 'eta' does not exist");
    assert(index < m_etaColumn->size() && "Index out of bounds");
    return m_etaColumn->proxy(*this)[index];
  }
  ...
  /// Const access to the etaR coordinates of the space point at the given index.
  /// @param index The index of the space point.
  /// @return A const reference to the etaR coordinates of the space point.
  const std::array<float, 2> &etaR(Index index) const noexcept {
    assert(m_etaRColumn.has_value() && "Column 'etaR' does not exist");
    assert(index < m_etaRColumn->size() && "Index out of bounds");
    return m_etaRColumn->proxy(*this)[index];
  }
 ...
 private:
  ...
  std::optional<ColumnHolder<float>> m_etaColumn;
  ...
  std::optional<ColumnHolder<std::array<float, 2>>> m_etaRColumn;
  ...
  static auto knownColumnDefaults() noexcept {
    return std::tuple(
        ...
        float{0}, float{0}, float{0}, float{NoTime}, float{0}, float{0},  // ← Add float{0} for eta
        std::array<float, 3>{0, 0, 0}, std::array<float, 3>{0, 0, 0},
        std::array<float, 3>{0, 0, 0}, std::array<float, 3>{0, 0, 0},
        SpacePointIndex2{0}, std::array<float, 2>{0, 0},
        std::array<float, 2>{0, 0}, std::array<float, 2>{0, 0},  // ← Add for etaR
  ...
  auto knownColumns() & noexcept {
    return std::tie(m_sourceLinkOffsetColumn, m_sourceLinkCountColumn,
                    m_xColumn, m_yColumn, m_zColumn, m_etaColumn, m_rColumn, m_phiColumn,  // ← Add m_etaColumn
                    ...
                    m_copyFromIndexColumn, m_xyColumn, m_zrColumn, m_etaRColumn, m_xyzColumn,  // ← Add m_etaRColumn
                    m_xyzrColumn, m_varianceZRColumn);
  }
  auto knownColumns() const & noexcept {
    return std::tie(m_sourceLinkOffsetColumn, m_sourceLinkCountColumn,
                    m_xColumn, m_yColumn, m_zColumn, m_etaColumn, m_rColumn, m_phiColumn,
                    m_timeColumn, m_varianceZColumn, m_varianceRColumn,
                    m_topStripVectorColumn, m_bottomStripVectorColumn,
                    m_stripCenterDistanceColumn, m_topStripCenterColumn,
                    m_copyFromIndexColumn, m_xyColumn, m_zrColumn, m_etaRColumn, m_xyzColumn,
                    m_xyzrColumn, m_varianceZRColumn);
  }
  auto knownColumns() && noexcept {
    return std::tuple(
        std::move(m_sourceLinkOffsetColumn), std::move(m_sourceLinkCountColumn),
        std::move(m_xColumn), std::move(m_yColumn), std::move(m_zColumn), std::move(m_etaColumn),
        ...
        std::move(m_xyColumn), std::move(m_zrColumn), std::move(m_etaRColumn), std::move(m_xyzColumn),
