...
#include "Acts/Utilities/stats-helper.h"
...
static inline bool itkFastTrackingCuts(
  ...
  static constexpr float rMin = 45.0;
  static constexpr float cotThetaMax = 1.5;
...
ProcessCode SphericalGridTripletSeedingAlgorithm::execute(
  ...
  STATS(grid_eta_bins);
  STATS(grid_r_bins);
  STATS(middle_bins_processed);
  STATS(middle_bins_skipped_empty);
  STATS(seeds_total);

  Acts::SphericalSpacePointGrid2 grid(m_gridConfig,
                                        logger().cloneWithSuffix("Grid"));

  const std::size_t nEta = m_gridConfig.etaBinEdges.empty()
      ? static_cast<std::size_t>(std::max(1.f, std::floor(
            (m_gridConfig.etaMax - m_gridConfig.etaMin) /
            m_gridConfig.deltaEtaMax)))
      : (m_gridConfig.etaBinEdges.size() - 1u);
  const std::size_t nR = m_gridConfig.rBinEdges.empty()
      ? 1u
      : (m_gridConfig.rBinEdges.size() - 1u);
  const std::size_t nPhi = grid.numberOfBins() / ((nEta + 2u) * (nR + 2u));
  grid_phi_bins(static_cast<std::int64_t>(nPhi));
  grid_eta_bins(static_cast<std::int64_t>(nEta));
  ...
  for (std::size_t i = 0; i < spacePoints.size(); ++i) {
    ...
    float eta = std::asinh(sp.z() / r);

    grid.insert(i, phi, eta, sp.r());
    sp_inserted(1);
  }
  ...
  seeds_total(static_cast<std::int64_t>(seeds.size()));
...
SphericalGridTripletSeedingAlgorithm::retrieveRadiusRangeForMiddle(
  ...
  // get etaBin position of the middle SP
  auto pVal = std::ranges::lower_bound(m_cfg.etaBinEdges, spM.etaR()[0]);
