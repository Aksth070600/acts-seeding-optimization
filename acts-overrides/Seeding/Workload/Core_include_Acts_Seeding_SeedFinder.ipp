...
void SeedFinder<external_spacepoint_t, grid_t, platform_t>::createSeedsForGroup(
  ...
  STATS(bottom_candidates);
  STATS(seeds_after_filter);
  ...
  for (const external_spacepoint_t* spM : middleSPs) {
    ...
    top_candidates(static_cast<std::int64_t>(state.compatTopSP.size()));
    ...
    bottom_candidates(static_cast<std::int64_t>(state.compatBottomSP.size()));
    ...
    const std::size_t seedsBefore = outputCollection.size();
    ...
    seeds_after_filter(
        static_cast<std::int64_t>(outputCollection.size() - seedsBefore));
