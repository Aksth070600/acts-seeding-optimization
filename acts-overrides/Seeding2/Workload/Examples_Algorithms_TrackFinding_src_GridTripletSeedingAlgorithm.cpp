...
ProcessCode GridTripletSeedingAlgorithm::execute(
  ...
  const std::size_t nZ = m_gridConfig.zBinEdges.empty()
      ? static_cast<std::size_t>(std::max(1.f, std::floor(
            (m_gridConfig.zMax - m_gridConfig.zMin) /
            (m_gridConfig.cotThetaMax * m_gridConfig.deltaRMax))))
      : (m_gridConfig.zBinEdges.size() - 1u);
  const std::size_t nR = m_gridConfig.rBinEdges.empty()
      ? 1u
      : (m_gridConfig.rBinEdges.size() - 1u);
  const std::size_t nPhi = grid.numberOfBins() / ((nZ + 2u) * (nR + 2u));
  ...
  for (const auto& [bottom, middle, top] : grid.binnedGroup()) {
    const auto sz = grid.at(middle).size();
    num_bins(1);
    if (sz > 0) {
      occupied_bins(1);
    }
  ...
  for (const auto [bottom, middle, top] : grid.binnedGroup()) {
    ...
    const std::size_t seedsBefore = seeds.size();
