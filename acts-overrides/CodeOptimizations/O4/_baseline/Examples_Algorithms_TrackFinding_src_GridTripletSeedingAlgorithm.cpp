...
ProcessCode GridTripletSeedingAlgorithm::execute(
  ...
  // O4: Reserve each grid bin upfront using the average bin occupancy as a
  // capacity estimate. 
  {
    const std::size_t nBins = grid.numberOfBins();
    const std::size_t avgBinOccupancy =
        (spacePoints.size() + nBins - 1) / nBins;
    for (std::size_t i = 0; i < nBins; ++i) {
      grid.at(i).reserve(avgBinOccupancy);
    }
  }
