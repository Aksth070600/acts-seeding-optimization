...
class Impl final : public TripletSeedFinder {
 public:
  ...
  void createPixelTripletTopCandidates(
    ...
    TIMER("createPixelTripletTopCandidates");
    ...
    for (auto [topDoublet, topDoubletIndex] :
      ...
      // O2: Compute deltaCotTheta2 first — it is a single subtraction and
      // multiply, costing ~40 ms pixel in profiling. The full error2
      // computation costs ~1.85 s pixel and is only needed when
      // deltaCotTheta2 exceeds scatteringInRegion2. Since error2 >= 0 by
      ...
      // The cheap pre-filter below therefore gates the expensive error2
      // computation without changing the set of accepted candidates.
      const float deltaCotTheta = cotThetaB - cotThetaT;
      const float deltaCotTheta2 = deltaCotTheta * deltaCotTheta;

      // use geometric average (moved after cheap deltaCotTheta2 computation)
      const float cotThetaAvg2 = cotThetaB * cotThetaT;

      // Pre-filter: skip error2 computation entirely when deltaCotTheta2 is
      // already within the scattering-only budget. error2 >= 0 guarantees
      // this cannot incorrectly reject a candidate.
      if (deltaCotTheta2 > scatteringInRegion2) {
        ...
        // the  space-point-related squared error (error2) and a scattering
        // term calculated assuming the minimum pt we expect to reconstruct
      ...
      // Note: error2 is recomputed here only if the pre-filter above was
      // skipped (deltaCotTheta2 <= scatteringInRegion2). In that case
      // deltaCotTheta2 <= scatteringInRegion2 implies this second cut also
      // passes unless p2scatterSigma is negative, which cannot occur.
      // For the path where the pre-filter fired and error2 was computed,
      // we reuse cotThetaAvg2 which remains in scope.
      const float error2 = topDoublet.er() + erB +
                           2 * (cotThetaAvg2 * varianceRM + varianceZM) *
                               iDeltaRB * topDoublet.iDeltaR();
