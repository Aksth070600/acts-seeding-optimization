...
DATA_DIR = REPO / "raw-data" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "DeltaEtaMax"
BASE_DIR = REPO / "raw-data" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "Baseline"
OUTPUT   = REPO / "figures" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "DeltaEtaMax.pdf"

VALUES = load_config("parameter_optimization")["sweeps_1d"]["deltaEtaMax"]
...
def main() -> None:
    ...
    plot_parameter_scan(
        ...
        dup   = grids["dup"],
        time_ms = grids["time"],
        parameter_label = r"$\Delta\eta_\mathrm{max}$",
        title = r"Seed-finding performance vs. $\eta$-bin width",
        output_path = OUTPUT,
        baseline = base,
        anchor_x = 0.25,
    )
