...
DATA_DIR = REPO / "raw-data" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "ImpactMax"
BASE_DIR = REPO / "raw-data" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "Baseline"
OUTPUT   = REPO / "figures" / "Results" / "AlgorithmOptimizations" / "ParameterOptimization" / "ImpactMax.pdf"

VALUES = load_config("parameter_optimization")["sweeps_1d"]["impactMax"]
...
def main() -> None:
    ...
    plot_parameter_scan(
        ...
        dup   = grids["dup"],
        time_ms = grids["time"],
        parameter_label = "impactMax [mm]",
        title = "Seed-finding performance vs. max impact parameter",
        output_path = OUTPUT,
        baseline = base,
        xscale = "log",
        anchor_x = 20,
