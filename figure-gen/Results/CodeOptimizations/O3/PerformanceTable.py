...
CALLGRIND_TARGETS = {
    "Pixel": ["sortByCotTheta"],
    "Strip": ["sortByCotTheta"],
}
...
def detector_rows(df_global, geom: str, seed_offset: int, title: str) -> list[str]:
    ...
    emit(r"\texttt{sortByCotTheta} (Ir)",           *_cg_row(cg, "Ir", "local"))
    if has_bcm:
        emit(r"Global branch mispred.\ (Bcm)",      *_cg_row(cg, "Bcm", "global"))
        emit(r"\texttt{sortByCotTheta} mispred.\ (Bcm)", *_cg_row(cg, "Bcm", "local"))

    return rows


def build_latex_table(df_global) -> str:
    lines: list[str] = [
    ...
    lines += detector_rows(df_global, "Pixel", 0,    "Pixel detector")
    lines += detector_rows(df_global, "Strip", 1000, "Strip detector")
...
def main() -> None:
    ...
    save_tex(OUTPUT_FILE, build_latex_table(df_global))
